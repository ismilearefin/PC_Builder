...
export default function PcBuilder({product,selectedProduct}) {
    ...
    <div className="border min-h-[150px] px-2 md:px-0">
        ...
        <p className='text-8xl'>{Items.length > 0 ? <Image src={Items[0]?.image} alt='logo' width={100} height={100}></Image> : product.icon}</p>
        <div className='w-2/3'>
        <p className='text-lg md:text-2xl'>{product.name} <span className='bg-gray-400 text-sm text-white px-2 md:px-4 md:py-0.5 md:rounded'>Required</span></p>
            
            { Items.length ?
                <div className='pt-1'>
                <div className='flex justify-between'>
                <p className='md:text-lg'>{Items[0]?.productName}</p>
                <p className='md:border md:py-1 px-2 md:bg-gray-100 rounded-sm font-semibold'>${Items[0]?.price}</p>
                </div>
                <div className='flex gap-x-6 justify-between md:justify-normal'>
                    <p className='text-[#358022] font-semibold'>{Items[0]?.status}</p>
